...
public class IntegerVariable extends AbstractVariable<Integer> {
	...
	public IntegerVariable(String name, String id, ScopeType scopeType) {
		super(name, id, VariableType.INTEGER, scopeType);
	}
	
	@Override
	public void addConstant(String attributeConstant) {
		constants.add(Integer.valueOf(attributeConstant));
	}
	...
	public Set<String> getMatchingPropositionNames(String atomicCondition, boolean matchGlobalVariableValue) {
		...
		if (!splitEquality[0].equals(getName()) ) {
			...
			//Using px for this case might not be the best choice, but if we reach here then something is already wrong
			System.err.println("Condition constant (" + atomicCondition + ") is not an integer. Using px as proposition");
			propositionNames.add(getId() + "px");
		} else {
			int propositionId = getPropositionIndex(Integer.valueOf(splitEquality[1])); //Proposition that matches the constant used in the atomic condition
	...
	public String getPropositionValue(int propositionId) { //From old implementation, can probably be improved
		...
		if (propositionId == 0) {
			...
			if (singleValueInterval) {
				int index = 1;
				for (Integer conditionCalue : constants) {
	...
	private int getPropositionIndex(Integer value) {
		...
		for (Integer conditionValue : constants) {
			index++;
